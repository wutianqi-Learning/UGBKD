...
model = dict(
    ...
    distiller=dict(
        ...
        student_recorders=dict(
            up_layer1=dict(type=ModuleOutputsRecorder, source='segmentor.backbone.bu_dec_l2'),#24
            up_layer2=dict(type=ModuleOutputsRecorder, source='segmentor.backbone.bu_dec_l3'),#48
            up_layer3=dict(type=ModuleOutputsRecorder, source='segmentor')#96
        ),
        teacher_recorders=dict(
            up_layer1=dict(type=ModuleOutputsRecorder, source='segmentor.backbone.decoder3'),#48
            up_layer2=dict(type=ModuleOutputsRecorder, source='segmentor.backbone.decoder2'),#96
            up_layer3=dict(type=ModuleOutputsRecorder, source='segmentor')#96
        ),
        distill_losses=dict(
            loss_cakd1=dict(
