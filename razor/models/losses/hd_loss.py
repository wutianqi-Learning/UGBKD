...
if __name__ == '__main__':
    ...
    gt_labels = one_hot(gt_labels, num_classes=14)
    loss_hd = HausdorffDTLoss(softmax=False)
    # feats_S = torch.rand(1, 14, *gt_labels.shape)
    # feats_T = torch.rand(1, 14, *gt_labels.shape)
    loss = loss_hd(gt_labels, gt_labels, gt_labels)
    ...
    # affine = nib.load(label_path).affine
    #
    # target_dt_ = target_dt.cpu().numpy().astype(np.uint8)
    # pred_dt_S_ = pred_dt_S.cpu().numpy().astype(np.uint8)
    #
    # save_dirs = os.path.join(
    #     'save_dirs', 'hd',
    #     f"{i}_{osp.basename(label_path.replace('label', 'target_dt'))}")
    # nib.save(
    #     nib.Nifti1Image(target_dt_.astype(np.uint8), affine),
    #     save_dirs)
    #
    # save_dirs = os.path.join(
    #     'save_dirs', 'hd',
    #     f"{i}_{osp.basename(label_path.replace('label', 'pred_dt_S'))}")
    # nib.save(
    #     nib.Nifti1Image(pred_dt_S_.astype(np.uint8), affine),
    #     save_dirs)