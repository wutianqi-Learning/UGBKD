...
from seg.models.segmentors.monai_model import *

from razor.models.algorithms.distill.configurable.single_teacher_distill import SingleTeacherDistill_ADE
from .unet_base_sgd_synapse_dae import model as teacher_model
from mmengine.runner import load_checkpoint
dae_ckpt = '/home/s316/workspace2/zhangdw/monai_mmengine/ckpts/unetmod_base_d8_1000e_sgd_synapse_96x96x96/best_Dice_81-69_epoch_800.pth'

def load_DAE_model():
        ade = SingleTeacherDistill_ADE(
            architecture=dict(cfg_path=teacher_model, pretrained=False),
...
class MonaiMetric(BaseMetric):
    ...
    def save_predictions(outputs: torch.Tensor, data_samples: dict, output_dir: str) -> None:
        ...
        inferer_dae = partial(
                ...
                predictor=load_DAE_model(),
                overlap=infer_overlap)
        # get output of teacher model
        outputs = torch.softmax(outputs, 1)
        outputs = torch.argmax(outputs, 1)
        
        dea_input_argmaxed = outputs[:, None, :, :, :].type(torch.FloatTensor).cuda()
        dae_outputs = inferer_dae(dea_input_argmaxed)
        dae_outputs = torch.softmax(dae_outputs, 1)
        dae_outputs = torch.argmax(dae_outputs, 1)
        ...
        dae_outputs = dae_outputs.cpu().numpy().astype(np.uint8)[0]
        dae_outputs_outputs = resample_3d(dae_outputs, shape)
        
        outputs = outputs.cpu().numpy().astype(np.uint8)[0]
        val_outputs = resample_3d(outputs, shape)

        dae_save_dirs = os.path.join(
            output_dir,'DAE'+\
            osp.basename(img_path.replace('label', 'pred')))
        nib.save(
            nib.Nifti1Image(dae_outputs_outputs.astype(np.uint8), affine),
            dae_save_dirs)
