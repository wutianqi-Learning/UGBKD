...
class Generic_TransUNet_max_ppbp(SegmentationNetwork):
    ...
    def __init__(self, input_channels, base_num_features, num_classes, num_pool, num_conv_per_stage=2,
                 ...
                 point_rend=False, num_point_rend=None, no_object_weight=None, is_mhsa_float32=False,
                 no_max_hw_pe=False,
        ...
        if self.is_max_bottleneck_transformer:
            ...
            feat_size = [int(self.patch_size[0] / 2 ** num_pool_per_axis[0]),
                         int(self.patch_size[1] / 2 ** num_pool_per_axis[1]),
                         int(self.patch_size[2] / 2 ** num_pool_per_axis[2])]
            self.transformer = Transformer(config_vit, feat_size=feat_size, vis=False, feat_channels=output_features,
        ...
        if self.is_max:
            ...
            if self.is_max_ms:  # use multi-scale feature as Transformer decoder input
                if self.is_max_ms_fpn:
                    for idx, in_channels in enumerate(
                            decoder_channels[:max_n_fpn]):  # max_n_fpn=4: 1/32, 1/16, 1/8, 1/4
                        input_proj_list.append(nn.Sequential(
                            nn.Conv3d(in_channels, max_hidden_dim, kernel_size=1),
                            nn.GroupNorm(32, max_hidden_dim),
                            nn.Upsample(size=(int(patch_size[0] / 2), int(patch_size[1] / 4), int(patch_size[2] / 4)),
                                        mode='trilinear')
                        ))  # proj to scale (1, 1/2, 1/2), TODO: init
                    self.input_proj = nn.ModuleList(input_proj_list)
                    self.linear_encoder_feature = nn.Conv3d(max_hidden_dim * max_n_fpn, max_hidden_dim, 1,
                                                            1)  # concat four-level feature
                else:
                ...
                self.linear_mask_features = nn.Conv3d(decoder_channels[-1], cfg["mask_dim"], kernel_size=1, stride=1,
                                                      padding=0, )  # following SingleScale, high-level feat, obtain seg_map
            else:
                self.linear_encoder_feature = nn.Conv3d(decoder_channels[max_ss_idx], cfg["mask_dim"], kernel_size=1)
                self.linear_mask_features = nn.Conv3d(decoder_channels[-1], cfg["mask_dim"], kernel_size=1, stride=1,
                                                      padding=0, )  # low-level feat, dot product Trans-feat
            ...
            if self.is_masked_attn:
                ...
                self.predictor = MultiScaleMaskedTransformerDecoder3d(in_channels=max_hidden_dim,
                                                                      mask_classification=is_max_cls, **cfg)
            else:
                from .mask2former_modeling.transformer_decoder.mask2former_transformer_decoder3d import \
                    StandardTransformerDecoder
                cfg["dropout"], cfg["enc_layers"], cfg["deep_supervision"] = 0.1, 0, False
                self.predictor = StandardTransformerDecoder(in_channels=max_hidden_dim, mask_classification=is_max_cls,
                                                            **cfg)
    ...
    def forward(self, x):
        ...
        for u in range(len(self.tu)):
            if u < len(self.tu) - 1 and isinstance(self.is_fam, str) and self.is_fam.startswith('fam_down'):
                skip_down = nn.Upsample(size=x.shape[2:])(skips[-(u + 1)]) if x.shape[2:] != skips[-(u + 1)].shape[
                                                                                             2:] else skips[-(u + 1)]
        ...
        if self.is_max:
            if self.is_max_ms:  # is_max_ms_fpn
                ...
                ms_pixel_feats = ds_feats[:self.max_n_fpn] if self.is_max_ms_fpn else [ds_feats[i] for i in
                                                                                       self.max_ms_idxs]

                for idx, f in enumerate(ms_pixel_feats):
                    f = self.input_proj[idx](
                        f)  # proj into same spatial/channel dim , but transformer_decoder also project to same mask_dim
                    multi_scale_features.append(f)
                transformer_decoder_in_feature = self.linear_encoder_feature(torch.cat(multi_scale_features,
                                                                                       dim=1)) if self.is_max_ms_fpn else multi_scale_features  # feature pyramid
            ...
            if self.is_max_cls and self.is_max_ds:
                if self._deep_supervision and self.do_ds:
                    return [predictions] + [i(j) for i, j in
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        for b in range(bs):
            ...
            tgt_mask = targets[b]["masks"].to(
                out_mask)  # [K, D, H, W], K is number of classes shown in this image, and K < n_class
