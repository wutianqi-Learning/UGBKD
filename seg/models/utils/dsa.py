...
class SelectiveKernelAttn(nn.Module):
    def __init__(self, channels, num_paths=2, attn_channels=32, act_layer=nn.ReLU, norm_layer=nn.BatchNorm3d):
        ...
        self.fc_reduce = nn.Conv3d(channels, attn_channels, kernel_size=1, bias=False)
        self.bn = norm_layer(attn_channels)
        self.act = act_layer(inplace=True)
        self.fc_select = nn.Conv3d(attn_channels, channels * num_paths, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        # [B, C, 1, 1, 1]
        x = x.sum(1).mean((2, 3, 4), keepdim=True)
        ...
        B, C, H, W, D = x.shape
        # [B, 2, C, 1, 1, 1]
        x = x.view(B, self.num_paths, C // self.num_paths, H, W, D)
        # [B, 2, C, 1, 1, 1]
...
class DSA_V13(nn.Module):
    ...
    def __init__(self,
        ...
        if fusion_type in ['dsa', 'dam']:
            ...
            if fusion_type == 'dsa':
                self.conv_out = nn.Sequential(
                    nn.Conv3d(self.in_channels, self.channels, kernel_size=1),
                    nn.LayerNorm([self.channels, 1, 1, 1]),
                    nn.ReLU(inplace=True),  # yapf: disable
                    nn.Conv3d(self.channels, self.in_channels, kernel_size=1))
        if fusion_type in ['pr', 'sq', 'dam']:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if 'sp' in self.attn_types:
            ...
            # print(f'spatial_attn: {spatial_attn.reshape(b, h * w)}')
            if 'ch' not in self.attn_types:
                spatial_attn = spatial_attn.reshape(b, 1, h, w, d)
                out = x * spatial_attn
                return out
            spatial_attn = spatial_attn.reshape(b, 1, h * w * d)
        else:
        ...
        if self.fusion_type == 'sq' or self.fusion_type in ['dsa', 'dam']:
            ...
            # sequence = torch.softmax(sequence, 1)
            # [B, C, H, W, D]
            sequence = sequence.reshape(b, c, h, w, d)
            if self.fusion_type not in ['dsa', 'dam']:
                out = x * self.combine_attn(sequence)
                return out
        else:
        ...
        if self.fusion_type == 'pr' or self.fusion_type in ['dsa', 'dam']:
            ...
            # parallel = torch.softmax(parallel, 1)
            # [B, C, H, W]
            parallel = parallel.reshape(b, c, h, w, d)
            if self.fusion_type not in ['dsa', 'dam']:
                out = x * self.combine_attn(parallel)
                return out
            else:
                ...
                # sq_pr = self.sk(sequence, parallel)
                # [B, C, H * W * D]
                sq_pr = sq_pr.reshape(b, c, h * w * d)

                self_attn = torch.softmax(sq_pr, 2)
                # [B, C, H, W, D]
                self_attn = self_attn.reshape(b, c, h, w, d)

                # v13, remove res_conv
                # [B, C, H, W, D]
                value = x * self_attn
                # [B, C, 1, 1, 1]
                aggregation = value.reshape(b, c, h * w * d).sum(dim=2).reshape(b, c, 1, 1, 1)
                # pool = self.avg_pool(self_attn).reshape(b, c, 1, 1)
                out = self.conv_out(aggregation)
