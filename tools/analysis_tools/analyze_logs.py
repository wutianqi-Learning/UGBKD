...
def plot_phase_val(metric, val_logs, curve_label):
    ...
    xs = np.array([log['step'] for log in val_logs[:-1]])
    ys = np.array([log[metric] for log in val_logs[:-1]])
    font_family_aliases = {
        'serif',
        'sans-serif',
        'sans serif',
    ...
    family = 'serif'
    plt.xlabel('Epochs', fontsize=20, family=family)
    plt.ylabel(metric, fontsize=20, family=family)
    linewidth = 2
    if curve_label == 'Ours':
        plt.plot(xs, ys, label=curve_label, linewidth=linewidth, marker='o', markersize=5, color='b')
    else:
        plt.plot(xs, ys, label=curve_label, linewidth=linewidth, marker='o', markersize=5, linestyle="--")
...
def main():
    ...
    isdir = False
    if os.path.isdir(json_logs[0]) and len(json_logs) == 1:
        base_dir = json_logs[0]
        json_logs = os.listdir(json_logs[0])
        json_logs = [os.path.join(base_dir, json_log) for json_log in json_logs if json_log.endswith('.json')]
    ...
    for json_log in json_logs:
        ...
        if isdir:
            cur_legend = os.path.basename(json_log).split('_')[0].upper()
            if cur_legend == 'MULTISCALE':
                cur_legend = 'Ours'
            elif cur_legend == 'BPKDV3':
                cur_legend = 'BPKD'
            elif cur_legend == 'ESPNETV2':
                cur_legend = 'Student(ESPNetV2)'
            elif cur_legend == 'UNET':
                cur_legend = 'Student(U-Net)'
    ...
    if args.task == 'cal_train_time':
        ...
        args.keys = ['Dice']
        # args.out = os.path.join(base_dir, f'{args.keys[0]}.svg')
