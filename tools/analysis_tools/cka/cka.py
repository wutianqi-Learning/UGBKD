...
from razor.models.losses.boundary_loss import get_edges
...
def plot(x: torch.Tensor):
    ...
    plt.imshow(x.cpu().numpy(), cmap='inferno', interpolation='nearest', aspect='auto')
    plt.colorbar()
    ...
    def calculate_cka_matrix(self) -> torch.Tensor:
        ...
        for epoch in range(self.num_epochs):
            ...
            for idx, data_batch in enumerate(loader):
                ...
                logits1 = self.model1._run_forward(data, mode='tensor')  # type: ignore
                logits2 = self.model2._run_forward(data, mode='tensor')  # type: ignore

                gt_labels = data['data_samples']['label']
                boundary_1 = get_edges(gt_labels[0, 0].detach().clone(), label_idx=1)
                cor_a = torch.corrcoef(logits1[0].view(14, -1))
                plot(cor_a)
                cor_b = torch.corrcoef(logits2[0].view(14, -1))
                plot(cor_b)
                ...
                for i in range(14):
                    if i == 0:
                        ...
                        mask_target_bs_list.append(torch.zeros_like(gt_labels)[0])
                        continue

                    mask_labels = gt_labels[bs, 0].detach().clone() == i

                    mask_edges_i = get_edges(mask_labels, i)
                    mask_target_i = mask_labels

                    mask_edges_bs_list.append(mask_edges_i.unsqueeze(0))
                    mask_target_bs_list.append(mask_target_i.unsqueeze(0))

                mask_edges_bs = torch.concat(mask_edges_bs_list, 0)
                mask_target_bs = torch.concat(mask_target_bs_list, 0)

                cor_a_boundary_1 = torch.corrcoef((logits1[0]*boundary_1).view(14, -1))
                plot(cor_a_boundary_1)
                cor_b_boundary_1 = torch.corrcoef((logits2[0]*boundary_1).view(14, -1))
                plot(cor_b_boundary_1)
                target_mask_6 = gt_labels[0, 0] == 6
                cor_a_target_6 = torch.corrcoef((logits1[0]*target_mask_6).view(14, -1))
                plot(cor_a_target_6)
                cor_b_target_6 = torch.corrcoef((logits2[0]*target_mask_6).view(14, -1))
                plot(cor_b_target_6)
